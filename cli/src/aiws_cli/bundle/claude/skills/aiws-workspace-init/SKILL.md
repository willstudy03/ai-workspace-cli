...
`scripts/`, and `skills/` — into either a project's `.claude/` folder or the
user's global `~/.claude/` folder. Every folder is seeded with a
...
"set up agent workspace", "bootstrap .claude structure", "create the folder structure",
"set up Claude Code folders", "workspace init", "start a new agent workspace"
...
chosen target (`.claude/` for a single project, or `~/.claude/` globally) and
...
>
> - **[G] Global** — Available across all projects. Files go into `~/.claude/`.
> - **[P] Project** — Scoped to one project. Files go into `<project>/.claude/`.
>
...
Target root = `$HOME/.claude`. Confirm:
> ✅ Global target root ready: `~/.claude/`
...
If `NOT_FOUND`, report and re-ask. Target root = `<project-dir>/.claude`. Confirm:
> ✅ Project target root ready: `<project-dir>/.claude/`
...
1. Target → Project → `/home/alice/projects/api/.claude/`
...
**User:** "Set up the Claude Code folders globally."

1. Target → Global → `~/.claude/`
