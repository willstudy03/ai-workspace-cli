...
version: "1.0.0"
description: "Scaffolds the complete agent-skills workspace file structure (agents, codebases, docs, knowledge, references, scripts, skills) into a project's .github/ folder or the user's global ~/.copilot/ folder, seeding every folder with a standards-compliant example file so users get a well-organized, ready-to-use layout."
...
`scripts/`, and `skills/` — into either a project's `.github/` folder or the
user's global `~/.copilot/` folder. Every folder is seeded with a
...
"set up agent workspace", "bootstrap .github structure", "create the folder structure",
"set up copilot folders", "workspace init", "start a new agent workspace"
...
chosen target (`.github/` for a single project, or `~/.copilot/` globally) and
...
>
> - **[G] Global** — Available across all projects. Files go into `~/.copilot/`.
> - **[P] Project** — Scoped to one project. Files go into `<project>/.github/`.
>
...
Target root = `$HOME/.copilot`. Confirm:
> ✅ Global target root ready: `~/.copilot/`
...
If `NOT_FOUND`, report and re-ask. Target root = `<project-dir>/.github`. Confirm:
> ✅ Project target root ready: `<project-dir>/.github/`
...
1. Target → Project → `/home/alice/projects/api/.github/`
...
**User:** "Set up the copilot folders globally."

1. Target → Global → `~/.copilot/`
