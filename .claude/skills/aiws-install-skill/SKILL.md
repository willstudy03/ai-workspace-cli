...
description: "Guides users through installing skills, agents, scripts, or references from the agent-skills repo into their global Claude Code configuration (~/.claude/) or a project-based .claude/ folder, including automatic dependency resolution and pre-install sync with the latest main branch."
...
"install reference", "set up Claude Code skill", "how do I use this skill",
"install into my project", "add to global Claude Code", "install globally"
...
scripts, or references in their own Claude Code environment — either globally (available
...
>
> - **[G] Global** — Available across all projects. Files will go into `~/.claude/`
>   so Claude Code can discover them system-wide.
...
2. Construct the global target root: `$HOME/.claude`

3. Check whether `~/.claude` already exists:

```bash
ls -la "$HOME/.claude" 2>/dev/null || echo "NOT_FOUND"
```
...
> ✅ Global target directory ready: `~/.claude/`
...
3. Check whether `.claude/` exists inside the project:

```bash
ls "<project-dir>/.claude" 2>/dev/null || echo "NOT_FOUND"
```

4. If `.claude/` does **not** exist, create it:

```bash
mkdir -p "<project-dir>/.claude"
echo "Created .claude/ in project"
```

5. Confirm to the user:
> ✅ Project target directory ready: `<project-dir>/.claude/`
...
4. If a `.claude/skills/<skill-name>/SKILL.md` version also exists, note it —
   prefer copying that version as it is the form Claude Code loads automatically.
...
Files to copy:
  ...
  ✅ references/code-review-java/code-review-java.md
     → ~/.claude/references/code-review-java/code-review-java.md

  ✅ references/java-code-standard/java-code-standard.md
     → ~/.claude/references/java-code-standard/java-code-standard.md

Directories to create (if not existing):
  ~/.claude/skills/code-review/
  ~/.claude/references/code-review-java/
  ~/.claude/references/java-code-standard/
─────────────────────────────────────────────
Total: 3 file(s)
```
...
How to use:
  Claude Code will now automatically discover the installed skill.
  Open a Claude Code chat and say "review my code" or
...
### Project Install (`<project-dir>/.claude/`)
...
1. Identify: `skills/code-review` → global target `~/.claude/`
...
5. Create dirs: `~/.claude/skills/code-review/`, `~/.claude/references/code-review-java/`,
   `~/.claude/references/java-code-standard/`
...
1. Identify: `agents/developer` → project target `/home/alice/projects/my-api/.claude/`
2. Sync with `origin/main`
...
5. Create all necessary dirs under `/home/alice/projects/my-api/.claude/`
