...
"install into my project", "add knowledge to global Codex", "install knowledge globally"
...
entries in their own Codex environment — either globally (available across all
...
>   `~/.codex/knowledge/` so Codex can discover them system-wide.
> - **[P] Project** — Available only for one specific project. Files will go into
>   the project's `.codex/knowledge/` folder.
>
...
2. Construct the global target root: `$HOME/.codex/knowledge`

3. Check whether it already exists:

```bash
ls -la "$HOME/.codex/knowledge" 2>/dev/null || echo "NOT_FOUND"
```

4. If it does **not** exist, create it:

```bash
mkdir -p "$HOME/.codex/knowledge"
echo "Created ~/.codex/knowledge"
```

5. Confirm to the user:
> ✅ Global target directory ready: `~/.codex/knowledge/`
...
3. Check whether `.codex/knowledge/` exists inside the project:

```bash
ls "<project-dir>/.codex/knowledge" 2>/dev/null || echo "NOT_FOUND"
```

4. If it does **not** exist, create it:

```bash
mkdir -p "<project-dir>/.codex/knowledge"
echo "Created .codex/knowledge/ in project"
```

5. Confirm to the user:
> ✅ Project target directory ready: `<project-dir>/.codex/knowledge/`
...
Target     : ~/.codex/knowledge/  (Global)

Files to copy:
  ✅ knowledge/concepts/example-idempotency.md
     → ~/.codex/knowledge/concepts/example-idempotency.md

  ✅ knowledge/policies/example-secret-management.md   (related)
     → ~/.codex/knowledge/policies/example-secret-management.md

Directories to create (if not existing):
  ~/.codex/knowledge/concepts/
  ~/.codex/knowledge/policies/
─────────────────────────────────────────────
...
How to use:
  Codex will now discover the installed knowledge entries. Ask a question that
...
### Global Install (`~/.codex/knowledge/`)
...
### Project Install (`<project-dir>/.codex/knowledge/`)
...
1. Identify: `knowledge/concepts/example-idempotency.md` → global target `~/.codex/knowledge/`
2. Sync with `origin/main`
3. Resolve `related` links → e.g. `policies/example-secret-management.md`
4. Show plan → user confirms **YES**
5. Create dirs: `~/.codex/knowledge/concepts/`, `~/.codex/knowledge/policies/`
...
1. Identify: `knowledge/workflows/example-incident-response.md` → project target `/home/alice/projects/my-api/.codex/knowledge/`
2. Sync with `origin/main`
3. Resolve all `related` entries recursively
4. Show full plan → user confirms **YES**
5. Create all necessary dirs under `.codex/knowledge/`
