...
**Location & naming**
- Path: `skills/<skill-name>/SKILL.md` or `.codex/skills/<skill-name>/SKILL.md`
...
6. **Location** — `.codex/skills/` (built-in, referenced by `AGENTS.md`) or `skills/` (repo library).
...
```bash
grep -rniE "<term1|term2|tool-name>" skills/ .codex/skills/ --include="*.md" -l
ls skills/ .codex/skills/
```
...
─────────────────────────────────────────────
Name       : <kebab-case-name>
Location   : .codex/skills/<name>/SKILL.md   (or skills/<name>/SKILL.md)
Objective  : <one line>
Triggers   : <phrases>
...
1. **Clarify** — objective (open an MR from the current branch), triggers
   ...
   name `gitlab-open-mr`, location `.codex/skills/`.
...
5. **Plan → YES**.
6. Author `.codex/skills/gitlab-open-mr/SKILL.md` with exact `glab` commands.
