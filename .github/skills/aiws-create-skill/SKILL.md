...
**Location & naming**
- Path: `skills/<skill-name>/SKILL.md` or `.github/skills/<skill-name>/SKILL.md`
...
6. **Location** — `.github/skills/` (built-in, auto-loaded) or `skills/` (repo library).
...
```bash
grep -rniE "<term1|term2|tool-name>" skills/ .github/skills/ --include="*.md" -l
ls skills/ .github/skills/
```
...
─────────────────────────────────────────────
Name       : <kebab-case-name>
Location   : .github/skills/<name>/SKILL.md   (or skills/<name>/SKILL.md)
Objective  : <one line>
Triggers   : <phrases>
...
1. **Clarify** — objective (open an MR from the current branch), triggers
   ...
   name `gitlab-open-mr`, location `.github/skills/`.
...
5. **Plan → YES**.
6. Author `.github/skills/gitlab-open-mr/SKILL.md` with exact `glab` commands.
