...
"install into my project", "add knowledge to global copilot", "install knowledge globally"
...
entries in their own Copilot environment — either globally (available across all
...
>   `~/.copilot/knowledge/` so Copilot can discover them system-wide.
> - **[P] Project** — Available only for one specific project. Files will go into
>   the project's `.github/knowledge/` folder.
>
...
2. Construct the global target root: `$HOME/.copilot/knowledge`

3. Check whether it already exists:

```bash
ls -la "$HOME/.copilot/knowledge" 2>/dev/null || echo "NOT_FOUND"
```

4. If it does **not** exist, create it:

```bash
mkdir -p "$HOME/.copilot/knowledge"
echo "Created ~/.copilot/knowledge"
```

5. Confirm to the user:
> ✅ Global target directory ready: `~/.copilot/knowledge/`
...
3. Check whether `.github/knowledge/` exists inside the project:

```bash
ls "<project-dir>/.github/knowledge" 2>/dev/null || echo "NOT_FOUND"
```

4. If it does **not** exist, create it:

```bash
mkdir -p "<project-dir>/.github/knowledge"
echo "Created .github/knowledge/ in project"
```

5. Confirm to the user:
> ✅ Project target directory ready: `<project-dir>/.github/knowledge/`
...
the latest version of each knowledge entry.
...
Target     : ~/.copilot/knowledge/  (Global)

Files to copy:
  ✅ knowledge/concepts/example-idempotency.md
     → ~/.copilot/knowledge/concepts/example-idempotency.md

  ✅ knowledge/policies/example-secret-management.md   (related)
     → ~/.copilot/knowledge/policies/example-secret-management.md

Directories to create (if not existing):
  ~/.copilot/knowledge/concepts/
  ~/.copilot/knowledge/policies/
─────────────────────────────────────────────
...
How to use:
  Copilot will now discover the installed knowledge entries. Ask a question that
...
### Global Install (`~/.copilot/knowledge/`)
...
### Project Install (`<project-dir>/.github/knowledge/`)
...
1. Identify: `knowledge/concepts/example-idempotency.md` → global target `~/.copilot/knowledge/`
2. Sync with `origin/main`
3. Resolve `related` links → e.g. `policies/example-secret-management.md`
4. Show plan → user confirms **YES**
5. Create dirs: `~/.copilot/knowledge/concepts/`, `~/.copilot/knowledge/policies/`
...
1. Identify: `knowledge/workflows/example-incident-response.md` → project target `/home/alice/projects/my-api/.github/knowledge/`
2. Sync with `origin/main`
3. Resolve all `related` entries recursively
4. Show full plan → user confirms **YES**
5. Create all necessary dirs under `.github/knowledge/`
